  ...
  return (
    <div className="space-y-8">
      <div className="flex justify-between items-center">
        <h1 className="text-4xl font-bold text-jungle-800">🌿 Mis Hábitos</h1>
        <button
          onClick={() => {
            setEditingHabit(null)
            setShowForm(true)
          ...
          <button
            ...
            className={`px-4 py-2 rounded-lg font-medium transition ${
              filter === f
                ...
                : 'bg-white text-jungle-700 hover:bg-jungle-100 border border-jungle-300'
            }`}
          >
      ...
      {filteredHabits.length === 0 ? (
        <div className="bg-white rounded-xl shadow-md p-12 text-center border-2 border-jungle-200">
          ...
          <p className="text-jungle-600 text-lg">
            {filter === 'all' 
              ? 'No tienes hábitos configurados aún'
              : `No tienes hábitos ${filter === 'daily' ? 'diarios' : filter === 'weekly' ? 'semanales' : 'mensuales'}`
            }
          </p>
        </div>
      ) : (
